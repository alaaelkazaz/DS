...
class Doctor :
    ...
    def Nextp(self,newp): # method to let the  next patient be the current patient and calculates the time he would take at the doctor's
        ...
        self.timeRemaining = newp.getages()//5
...
def ClinicSim(w):  # simulation for doctor clinic
    ...
    for currentsec in range(w):  # w is the time we enter ( 4 hours = 4*60 minute)
        if random.randrange(1,7)== 6 :                  #checks the probability of arrival of a patient at any given minute
